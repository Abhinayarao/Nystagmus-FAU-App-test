...
# ================= MAIN =================
video_path = '/content/drive/MyDrive/Dataset/14 January/IMG_2857.MOV'
cap = cv2.VideoCapture(video_path)

# Frames to display (only first 10 frames)
DISPLAY_FRAMES = [300, 302, 303, 304, 305, 306, 307, 308, 309, 310]
MAX_FRAME = 700  # Process 700 frames for tracking data
...
if len(tracking_data) > 0:
    ...
    # Save to CSV
    import csv
    csv_filename = 'eye_tracking_data_vertical_downward.csv'
    ...
    # ================= DETECT VERTICAL DECREASES OF -0.03 =================
    print(f"\n{'='*80}")
    print(f"DETECTING SIGNIFICANT VERTICAL MOVEMENTS (-0.03 decrease)")
    print(f"Phase 1: Connect all decrementing frames into continuous downward lines")
    print(f"Phase 2: Filter lines with total decrease ≥ 0.03")
    print(f"{'='*80}")

    # Phase 1: Find all continuous downward movement segments
    downward_segments = []
    current_segment = None

    for i in range(len(norm_y_values) - 1):
        y_current = norm_y_values[i]
        y_next = norm_y_values[i + 1]

        # Check if next point is lower (downward movement)
        if y_next < y_current:
            if current_segment is None:
                ...
                current_segment['end_y'] = y_next
        else:
            # Upward or flat movement - end current segment if exists
            if current_segment is not None:
                downward_segments.append(current_segment)
                current_segment = None
    ...
    print(f"\nPhase 1 Results:")
    print(f"  Total downward segments found: {len(downward_segments)}")

    # Phase 2: Filter segments by total decrease ≥ 0.03
    increase_regions = []

    for idx, segment in enumerate(downward_segments):
        total_decrease = segment['start_y'] - segment['end_y']  # Reversed for decrease
        ...
        print(f"    Y position: {segment['start_y']:.3f} → {segment['end_y']:.3f}")
        ...
        if total_decrease >= 0.03:
            increase_regions.append({
                ...
                'end_y': segment['end_y'],
                'increase': total_decrease,  # Store as positive value for consistency
                'num_frames': num_frames
            })
            print(f"    ✓ QUALIFIES (≥ 0.03) - Will be marked RED")
        else:
            print(f"    ✗ Too small (< 0.03) - Will remain GREEN")

    print(f"\nPhase 2 Results:")
    if len(increase_regions) == 0:
        print(f"  No downward segments with decrease ≥ 0.03 detected")
    else:
        print(f"  Segments with decrease ≥ 0.03: {len(increase_regions)}")
        print(f"  Average decrease: -{np.mean([r['increase'] for r in increase_regions]):.3f}")
        print(f"  Max decrease: -{np.max([r['increase'] for r in increase_regions]):.3f}")
        print(f"  Min decrease: -{np.min([r['increase'] for r in increase_regions]):.3f}")
    ...
    # Plot 3: Vertical Movement Analysis with Color-Coded Regions
    # Plot the line first
    ax3.plot(frames, norm_y_values, 'black', linewidth=2, marker='o', markersize=4, zorder=3)
    ...
    ax3.axhspan(0.3, 0.7, facecolor='lightgreen', alpha=0.3, zorder=1)

    # Highlight red regions where -0.03 decrease occurred (FAST PHASES)
    for region in increase_regions:
        ...
        mid_y = (region['start_y'] + region['end_y']) / 2
    ...
    if len(increase_regions) > 0:
        first_red_start_frame = increase_regions[0]['start_frame']
        ...
        # Draw connecting lines between consecutive red stripes
        for i in range(len(increase_regions) - 1):
            # End of current red stripe (FAST PHASE 1)
            end_frame_1 = increase_regions[i]['end_frame']
            end_y_1 = increase_regions[i]['end_y']

            # Start of next red stripe (FAST PHASE 2)
            start_frame_2 = increase_regions[i + 1]['start_frame']
            start_y_2 = increase_regions[i + 1]['start_y']

            # Draw connecting line (SLOW PHASE)
            ax3.plot([end_frame_1, start_frame_2], [end_y_1, start_y_2],
                    color='blue', linewidth=3, linestyle='-', alpha=0.7, zorder=5,
                    label='Slow Phase' if i == 0 else '')

            print(f"  Connected: Frame {end_frame_1} (y={end_y_1:.3f}) → Frame {start_frame_2} (y={start_y_2:.3f})")

        # Also draw line from excluded start point to first red stripe (if needed)
        if connection_start_frame < first_red_start_frame:
            # Find Y value at connection_start_frame
            start_idx = frames.index(connection_start_frame) if connection_start_frame in frames else 0
            start_y = norm_y_values[start_idx]
            first_red_y = increase_regions[0]['start_y']

            ax3.plot([connection_start_frame, first_red_start_frame], [start_y, first_red_y],
                    color='blue', linewidth=3, linestyle='-', alpha=0.7, zorder=5)

            print(f"  Initial connection: Frame {connection_start_frame} (y={start_y:.3f}) → Frame {first_red_start_frame} (y={first_red_y:.3f})")

        print(f"  Total slow phase connections drawn: {len(increase_regions)}")

    ax3.set_xlabel('Frame Number', fontsize=12, fontweight='bold')
    ax3.set_ylabel('Normalized Y Position', fontsize=12, fontweight='bold')
    ax3.set_title('Vertical Movement: Fast Phases (Red) + Slow Phases (Blue Lines) ≥0.03 Decrease',
                  fontsize=14, fontweight='bold')
    ...
    legend_elements = [
        ...
        Patch(facecolor='red', alpha=0.4, label='Fast Phase (≥0.03 decrease)'),
        Line2D([0], [0], color='blue', linewidth=3, alpha=0.7, label='Slow Phase (connection)'),
        Line2D([0], [0], color='r', linestyle='--', alpha=0.5, label='Center (0.5)')
    ]
    ax3.legend(handles=legend_elements, loc='upper right')

    # Add reference text
    info_text = f'Fast Phases: {len(increase_regions)} detected\nSlow Phases: Blue connecting lines'
    ...
    # Draw slow phase connections first (blue lines)
    if len(increase_regions) > 0:
        ...
        if connection_start_frame < first_red_start_frame:
            ...
            start_y = norm_y_values[start_idx]
            first_red_y = increase_regions[0]['start_y']
            ax4.plot([connection_start_frame, first_red_start_frame], [start_y, first_red_y],
                    color='blue', linewidth=3, linestyle='-', alpha=0.8, label='Slow Phase')
        ...
        for i in range(len(increase_regions) - 1):
            ...
            ax4.plot([end_frame_1, start_frame_2], [end_y_1, start_y_2],
                    color='blue', linewidth=3, linestyle='-', alpha=0.8)

    # Draw fast phase lines (red lines showing the downward segments)
    for i, region in enumerate(increase_regions):
        start_frame = region['start_frame']
        end_frame = region['end_frame']
        start_y = region['start_y']
        end_y = region['end_y']

        ax4.plot([start_frame, end_frame], [start_y, end_y],
                color='red', linewidth=3, linestyle='-', alpha=0.8,
                label='Fast Phase' if i == 0 else '')

    ax4.set_xlabel('Frame Number', fontsize=12, fontweight='bold')
    ax4.set_ylabel('Normalized Y Position', fontsize=12, fontweight='bold')
    ax4.set_title('Clean View: Fast Phases (Red - Downward) + Slow Phases (Blue) Only',
                  fontsize=14, fontweight='bold')
    ax4.grid(True, alpha=0.3)
    ax4.set_ylim(0.3, 0.7)
    ax4.legend(loc='upper right')

    print(f"  Plot 4 complete: {len(increase_regions)} fast phases + {len(increase_regions)} slow phase connections")

    plt.tight_layout()
    plt.savefig('eye_movement_plots_vertical_downward.png', dpi=150, bbox_inches='tight')
    print(f"✓ Movement plots with vertical downward analysis saved to 'eye_movement_plots_vertical_downward.png'")
    plt.show()
...
# Bridge variables from detection code to SPV code
fast_phase_regions = []
for r in increase_regions:
...
norm_x_values = norm_y_values  # SPV code expects norm_x_values
...
SPV_THRESHOLD = 30  # deg/sec - exclude SPV over this value for peak detection

# Calculate SPV for each slow phase (RIGHTWARD drift between leftward fast phases)
spv_data = []

if len(fast_phase_regions) > 0:  # CHANGED: increase_regions → fast_phase_regions
    ...
    if connection_start_frame < first_red_start_frame:
        start_idx = frames.index(connection_start_frame) if connection_start_frame in frames else 0
        start_x = norm_x_values[start_idx]  # CHANGED: norm_y_values → norm_x_values
        first_red_x = fast_phase_regions[0]['start_x']  # CHANGED: start_y → start_x
        ...
        spv_data.append({
            ...
            'start_x': start_x,  # CHANGED: start_y → start_x
            'end_x': first_red_x,  # CHANGED: end_y → end_x
            'num_frames': num_frames,
    ...
    for i in range(len(fast_phase_regions) - 1):  # CHANGED: increase_regions → fast_phase_regions
        end_frame_1 = fast_phase_regions[i]['end_frame']
        end_x_1 = fast_phase_regions[i]['end_x']  # CHANGED: end_y_1 → end_x_1
        start_frame_2 = fast_phase_regions[i + 1]['start_frame']
        start_x_2 = fast_phase_regions[i + 1]['start_x']  # CHANGED: start_y_2 → start_x_2

        num_frames = start_frame_2 - end_frame_1
        normalized_diff = abs(start_x_2 - end_x_1)  # CHANGED: y → x
        ...
        spv_data.append({
            ...
            'start_x': end_x_1,  # CHANGED: start_y → start_x
            'end_x': start_x_2,  # CHANGED: end_y → end_x
            'num_frames': num_frames,
            'normalized_diff': normalized_diff,
            'deg_diff': deg_diff,
            'time_sec': time_sec,
...
# Find peak SPV (only considering SPV ≤ 30 deg/sec)
peak_spv_data = None
if len(spv_data) > 0:
    # Filter to only include SPV values ≤ threshold
    valid_spv = [s for s in spv_data if s['spv'] <= SPV_THRESHOLD]

    if len(valid_spv) > 0:
        peak_spv_idx = np.argmax([s['spv'] for s in valid_spv])
        peak_spv_data = valid_spv[peak_spv_idx]

        print(f"\nPeak SPV detected (threshold: ≤{SPV_THRESHOLD} deg/sec):")
        ...
        excluded_spv = [s for s in spv_data if s['spv'] > SPV_THRESHOLD]
        if len(excluded_spv) > 0:
            print(f"\n  Excluded {len(excluded_spv)} beat(s) with SPV > {SPV_THRESHOLD} deg/sec:")
            for exc in excluded_spv:
                print(f"    Beat {exc['beat']}: {exc['spv']:.2f} deg/sec (EXCLUDED)")
    else:
        print(f"\nNo valid SPV values found (all beats exceed {SPV_THRESHOLD} deg/sec threshold)")
...
norm_x_in_deg = np.array(norm_x_values) * DEG_PER_NORMALIZED  # CHANGED: norm_y → norm_x
...
if len(fast_phase_regions) > 0:  # CHANGED: increase_regions → fast_phase_regions
    ...
    if connection_start_frame < first_red_start_frame:
        start_idx = frames.index(connection_start_frame) if connection_start_frame in frames else 0
        start_x_deg = norm_x_values[start_idx] * DEG_PER_NORMALIZED  # CHANGED: y → x
        first_red_x_deg = fast_phase_regions[0]['start_x'] * DEG_PER_NORMALIZED  # CHANGED: y → x
    ...
    for i in range(len(fast_phase_regions) - 1):
        end_frame_1 = fast_phase_regions[i]['end_frame']
        end_x_1_deg = fast_phase_regions[i]['end_x'] * DEG_PER_NORMALIZED  # CHANGED: y → x
        start_frame_2 = fast_phase_regions[i + 1]['start_frame']
        start_x_2_deg = fast_phase_regions[i + 1]['start_x'] * DEG_PER_NORMALIZED  # CHANGED: y → x
...
for i, region in enumerate(fast_phase_regions):  # CHANGED: increase_regions → fast_phase_regions
    start_frame = region['start_frame']
    end_frame = region['end_frame']
    start_x_deg = region['start_x'] * DEG_PER_NORMALIZED  # CHANGED: y → x
    end_x_deg = region['end_x'] * DEG_PER_NORMALIZED  # CHANGED: y → x
...
if peak_spv_data is not None:
    ...
    start_x_deg = peak['start_x'] * DEG_PER_NORMALIZED  # CHANGED: y → x
    end_x_deg = peak['end_x'] * DEG_PER_NORMALIZED  # CHANGED: y → x

    # Calculate box dimensions
    box_left = start_time
    box_width = end_time - start_time
    box_bottom = min(start_x_deg, end_x_deg)  # CHANGED: y → x
    box_height = abs(end_x_deg - start_x_deg)  # CHANGED: y → x
    ...
    mid_x_deg = (start_x_deg + end_x_deg) / 2  # CHANGED: y → x
...
ax5.set_ylabel('Vertical Position (degrees)', fontsize=12, fontweight='bold')  # CHANGED: added "Horizontal"
ax5.set_title(f'Vertical Nystagmus',
              fontsize=14, fontweight='bold')
ax5.grid(True, alpha=0.3)

# Set Y-axis limits (convert 0-1 normalized range to degrees for HORIZONTAL)
ax5.set_ylim(0, 1 * DEG_PER_NORMALIZED)  # CHANGED: 0.3-0.7 → 0-1
...
print(f"{'='*80}")
print(f"{'Beat':<6} {'Frames':<15} {'Time (sec)':<12} {'Deg Diff':<12} {'SPV (deg/sec)':<15} {'Status':<15}")
print(f"{'-'*80}")

for spv in spv_data:
    is_peak = "★ PEAK" if peak_spv_data and spv['beat'] == peak_spv_data['beat'] else ""
    is_excluded = "EXCLUDED" if spv['spv'] > SPV_THRESHOLD else ""
    status = is_peak if is_peak else is_excluded

    frame_range = f"{spv['start_frame']}-{spv['end_frame']}"
    print(f"{spv['beat']:<6} {frame_range:<15} {spv['time_sec']:<12.3f} {spv['deg_diff']:<12.2f} {spv['spv']:<15.2f} {status:<15}")
...
if len(spv_data) > 0:
    ...
    print(f"  Valid beats (≤{SPV_THRESHOLD} deg/sec): {len(valid_spv_values)}")
    print(f"  Excluded beats (>{SPV_THRESHOLD} deg/sec): {len(spv_data) - len(valid_spv_values)}")
