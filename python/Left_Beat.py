...
# ================= MAIN =================
video_path = '/content/drive/MyDrive/Dataset/13 January/IMG_2848.MOV'
...
print(f" 14. Detect LEFTWARD fast phases (decrease ≥0.04) and RIGHTWARD slow phases")
print("="*80)
...
if len(tracking_data) > 0:
    ...
    # ================= DETECT LEFTWARD FAST PHASES (DECREASES OF -0.04) =================
    print(f"\n{'='*80}")
    print(f"DETECTING SIGNIFICANT LEFTWARD MOVEMENTS (-0.04 decrease = FAST PHASE)")
    print(f"Phase 1: Connect all LEFTWARD (decrementing) frames into continuous lines")
    print(f"Phase 2: Filter lines with total DECREASE ≥ 0.04")
    print(f"{'='*80}")

    # Phase 1: Find all continuous LEFTWARD movement segments
    leftward_segments = []
    ...
    for i in range(len(norm_x_values) - 1):
        ...
        # Check if next point is further LEFT (leftward movement = DECREASE in x)
        if x_next < x_current:
            ...
            # Rightward or flat movement - end current segment if exists
            if current_segment is not None:
                leftward_segments.append(current_segment)
                current_segment = None
    ...
    print(f"\nPhase 1 Results:")
    print(f"  Total leftward segments found: {len(leftward_segments)}")

    # Phase 2: Filter segments by total DECREASE ≥ 0.04
    fast_phase_regions = []

    for idx, segment in enumerate(leftward_segments):
        total_decrease = segment['start_x'] - segment['end_x']  # CHANGED: start - end for leftward
    ...
    if len(fast_phase_regions) == 0:
        print(f"  No leftward segments with decrease ≥ 0.04 detected")
    else:
        print(f"  Fast phase segments (≥ 0.04 decrease): {len(fast_phase_regions)}")
        print(f"  Average decrease: -{np.mean([r['decrease'] for r in fast_phase_regions]):.3f}")
        print(f"  Max decrease: -{np.max([r['decrease'] for r in fast_phase_regions]):.3f}")
        print(f"  Min decrease: -{np.min([r['decrease'] for r in fast_phase_regions]):.3f}")
    ...
    ax1.plot(frames, norm_x_values, 'b-', linewidth=2, markersize=4)
    ...
    # Highlight red regions where -0.04 DECREASE occurred (FAST PHASES - leftward jumps)
    for region in fast_phase_regions:
        ...
        ax3.annotate(f"-{region['decrease']:.3f}\n({frames_text})",
                    xy=(mid_frame, mid_x),
                    fontsize=9,
                    fontweight='bold',
                    color='darkred',
    ...
    if len(fast_phase_regions) > 0:
        ...
        # Draw connecting lines between consecutive red stripes (SLOW PHASES - rightward drift)
    ...
    ax3.set_title('Horizontal Movement: Fast Phases (Red, Leftward ≥0.04) + Slow Phases (Blue, Rightward Drift)',
                  fontsize=14, fontweight='bold')
    ...
    legend_elements = [
        ...
        Patch(facecolor='red', alpha=0.4, label='Fast Phase (≥0.04 leftward decrease)'),
        Line2D([0], [0], color='blue', linewidth=3, alpha=0.7, label='Slow Phase (rightward drift)'),
        Line2D([0], [0], color='r', linestyle='--', alpha=0.5, label='Center (0.5)')
    ]
    ax3.legend(handles=legend_elements, loc='upper right')

    # Add reference text
    info_text = f'Fast Phases (Leftward): {len(fast_phase_regions)} detected\nSlow Phases (Rightward): Blue connecting lines'
    ...
    # Draw slow phase connections first (blue lines - rightward drift)
    if len(fast_phase_regions) > 0:
    ...
    # Draw fast phase lines (red lines showing the leftward segments)
    ...
    ax4.set_title('Clean View: Fast Phases (Red, Leftward) + Slow Phases (Blue, Rightward) Only',
                  fontsize=14, fontweight='bold')
...
SPV_THRESHOLD = 23  # deg/sec - exclude SPV over this value for peak detection
...
if peak_spv_data is not None:
    ...
    mid_x_deg = max(start_x_deg, end_x_deg) + padding_y * 0.4
...
# Add average SPV of non-excluded beats to top-left of graph
...
ax5.set_ylabel('Horizontal Position (degrees)', fontsize=12, fontweight='bold')  # CHANGED: added "Horizontal"
ax5.set_title(f'Horizontal Nystagmus',
              fontsize=14, fontweight='bold')
