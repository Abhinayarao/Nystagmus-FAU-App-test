...
MAX_FRAME = 700  # Process 500 frames for tracking data
...
if len(tracking_data) > 0:
    ...
    # ================= DETECT VERTICAL INCREASES OF +0.04 =================
    print(f"\n{'='*80}")
    print(f"DETECTING SIGNIFICANT VERTICAL MOVEMENTS (+0.04 increase)")
    print(f"Phase 1: Connect all incrementing frames into continuous upward lines")
    print(f"Phase 2: Filter lines with total increase ≥ 0.04")
    print(f"{'='*80}")

    # Phase 1: Find all continuous upward movement segments
    upward_segments = []
    ...
    for i in range(len(norm_y_values) - 1):
        ...
        # Check if next point is higher (upward movement)
        if y_next > y_current:
            ...
            # Downward or flat movement - end current segment if exists
            if current_segment is not None:
                upward_segments.append(current_segment)
                current_segment = None
    ...
    print(f"\nPhase 1 Results:")
    print(f"  Total upward segments found: {len(upward_segments)}")

    # Phase 2: Filter segments by total increase ≥ 0.03
    increase_regions = []

    for idx, segment in enumerate(upward_segments):
        total_increase = segment['end_y'] - segment['start_y']
        ...
        if total_increase >= 0.03:
            ...
            print(f"    ✓ QUALIFIES (≥ 0.04) - Will be marked RED")
        else:
            print(f"    ✗ Too small (< 0.04) - Will remain GREEN")

    print(f"\nPhase 2 Results:")
    if len(increase_regions) == 0:
        print(f"  No upward segments with increase ≥ 0.04 detected")
    else:
        print(f"  Segments with increase ≥ 0.04: {len(increase_regions)}")
        print(f"  Average increase: +{np.mean([r['increase'] for r in increase_regions]):.3f}")
        print(f"  Max increase: +{np.max([r['increase'] for r in increase_regions]):.3f}")
        print(f"  Min increase: +{np.min([r['increase'] for r in increase_regions]):.3f}")
    ...
    # Highlight red regions where +0.03 increase occurred (FAST PHASES)
    ...
    ax3.set_title('Vertical Movement: Fast Phases (Red) + Slow Phases (Blue Lines) ≥0.04',
                  fontsize=14, fontweight='bold')
    ...
    legend_elements = [
        ...
        Patch(facecolor='red', alpha=0.4, label='Fast Phase (≥0.04 increase)'),
    ...
    # Draw fast phase lines (red lines showing the upward segments)
    ...
    ax4.set_title('Clean View: Fast Phases (Red) + Slow Phases (Blue) Only',
                  fontsize=14, fontweight='bold')
    ...
    print(f"✓ Movement plots with vertical analysis saved to 'eye_movement_plots_with_analysis.png'")
    plt.show()
...
SPV_THRESHOLD = 20  # deg/sec - exclude SPV over this value for peak detection

# Calculate SPV for each slow phase
spv_data = []
...
if len(increase_regions) > 0:
    ...
    if connection_start_frame < first_red_start_frame:
        ...
        normalized_diff = abs(first_red_y - start_y)
    ...
    for i in range(len(increase_regions) - 1):
        ...
        normalized_diff = abs(start_y_2 - end_y_1)
...
norm_y_in_deg = np.array(norm_y_values) * DEG_PER_NORMALIZED

# Draw slow phase connections (blue lines)
if len(increase_regions) > 0:
    # Initial connection from excluded start to first fast phase
    first_red_start_frame = increase_regions[0]['start_frame']
    connection_start_frame = max(frames[0], first_red_start_frame - FRAMES_TO_EXCLUDE_BEFORE_FIRST)
    ...
    if connection_start_frame < first_red_start_frame:
        start_idx = frames.index(connection_start_frame) if connection_start_frame in frames else 0
        start_y_deg = norm_y_values[start_idx] * DEG_PER_NORMALIZED
        first_red_y_deg = increase_regions[0]['start_y'] * DEG_PER_NORMALIZED

        start_time = connection_start_frame / FPS
        end_time = first_red_start_frame / FPS

        ax5.plot([start_time, end_time], [start_y_deg, first_red_y_deg],
                color='blue', linewidth=3, linestyle='-', alpha=0.8, label='Slow Phase')

    # Connections between consecutive fast phases
    for i in range(len(increase_regions) - 1):
        end_frame_1 = increase_regions[i]['end_frame']
        end_y_1_deg = increase_regions[i]['end_y'] * DEG_PER_NORMALIZED
        start_frame_2 = increase_regions[i + 1]['start_frame']
        start_y_2_deg = increase_regions[i + 1]['start_y'] * DEG_PER_NORMALIZED

        end_time_1 = end_frame_1 / FPS
        start_time_2 = start_frame_2 / FPS

        ax5.plot([end_time_1, start_time_2], [end_y_1_deg, start_y_2_deg],
                color='blue', linewidth=3, linestyle='-', alpha=0.8)

# Draw fast phase lines (red lines)
for i, region in enumerate(increase_regions):
    start_frame = region['start_frame']
    end_frame = region['end_frame']
    start_y_deg = region['start_y'] * DEG_PER_NORMALIZED
    end_y_deg = region['end_y'] * DEG_PER_NORMALIZED

    start_time = start_frame / FPS
    end_time = end_frame / FPS

    ax5.plot([start_time, end_time], [start_y_deg, end_y_deg],
            color='red', linewidth=3, linestyle='-', alpha=0.8,
            label='Fast Phase' if i == 0 else '')
...
if peak_spv_data is not None:
    ...
    start_y_deg = peak['start_y'] * DEG_PER_NORMALIZED
    end_y_deg = peak['end_y'] * DEG_PER_NORMALIZED

    # Calculate box dimensions
    box_left = start_time
    box_width = end_time - start_time
    box_bottom = min(start_y_deg, end_y_deg)
    box_height = abs(end_y_deg - start_y_deg)
    ...
    mid_y_deg = (start_y_deg + end_y_deg) / 2

    ax5.annotate(f'Peak SPV\n{peak["spv"]:.2f} deg/sec',
                xy=(mid_time, mid_y_deg),
...
ax5.set_ylabel('Position (degrees)', fontsize=12, fontweight='bold')
ax5.set_title(f'Vertical Nystagmus',
              fontsize=14, fontweight='bold')
ax5.grid(True, alpha=0.3)

# Set Y-axis limits (convert 0.3-0.7 normalized range to degrees)
ax5.set_ylim(0.3 * DEG_PER_NORMALIZED, 0.7 * DEG_PER_NORMALIZED)
